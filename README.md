# A Two-Stage Learning Framework for Fine-Structure-Enhancedand Texture-Preserving CT Super-Resolution
Tong Lin#, Yikun Zhang*, Yang Chen, et al.


## Abstract
> High-resolution computed tomography (HRCT) is essential for precise disease diagnosis but remains inaccessible in many clinical settings due to hardware limitations and dose constraints. Previous work has demonstrated the promising performance of deep learning (DL) methods in volumetric super-resolution (SR). However, it is still challenging for existing methods to adequately restore fine structures and preserve image texture, especially in coronal and sagittal views. In this context, this study proposes a data-driven two-stage framework to progressively achieve fine-structureenhanced and texture-preserving CT super-resolution. In the structural super-resolution stage, a Wavelet-Guided Mamba Network is developed to perform volumetric SR. On the one hand, Swin Mamba (SWM) is integrated into network to overcome the limitations of convolutional neural networks (CNNs) in capturing long-range dependencies, enabling efficient modeling of volumetric contextual information while maintaining high inference efficiency. On the other hand, network is supervised by a waveletguided joint loss to ensure frequency consistency across multiple orientations between the predicted volume and HRCT, thereby enhancing fine structure recovery. In the textural enhancement stage, an adversarial learning mechanism is adopted to improve the consistency of texture distribution between the predicted volume and HRCT in the coronal view, thereby achieving texturepreserving SR. Quantitative and qualitative evaluations on three publicly available datasets demonstrate the promising performance of the proposed method in fine structure restoration and texture preservation while maintaining inference efficiency.
...
This repository is an implementation and extended adaptation of A Two-Stage Learning Framework for Fine-Structure-Enhanced and Texture-Preserving CT Super-Resolution, with additional refinements in code structure, documentation, and model reproducibility.
