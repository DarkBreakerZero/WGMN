...
def valid(valid_loader, g_model, writer, epoch):
    ...
    for data in tqdm(valid_loader, desc=f'Epoch {epoch}', unit='batch'):
...
if __name__ == "__main__":
    ...
    result_path = '/home/lintong/MBSRN/logs/'
    save_dir = '/models/'

    # Get dataset
    # train_dataset = read_deicom_file.dicom_reader(paired_img_txt='./train_img.txt')
    train_dataset = SRDataset("/mnt/no3/lintong/Texture_Preserver","test")
    train_loader = DataLoader(train_dataset, batch_size=4, num_workers=6, shuffle=True)

    valid_dataset = SRDataset("/mnt/no3/lintong/Texture_Preserver","test")
    valid_loader = DataLoader(valid_dataset, batch_size=1, num_workers=6, shuffle=True)
