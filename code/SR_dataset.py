...
class SRDataset(BaseDataset):
    def __init__(self, path,mode):
        ...
        # self.q_path_list=sorted_list(opt.dataroot+'/'+opt.phase+'/quarter_1mm_yiying/*')
        # self.f_path_list=sorted_list(opt.dataroot+'/'+opt.phase+'/full_1mm_yiying/*')
        # print(opt.phase)
        self.q_path_list = sorted_list(path + '/' + mode + '/sparse/*')
        self.f_path_list = sorted_list(path + '/' + mode + '/full/*')
    ...
    def __getitem__(self, index):
        ...
        q_data = np.expand_dims(q_data, axis=0)
