...
def reduce_mean(tensor, nprocs):  # The gradient is summarized and averaged
...
def get_edges_3d(input, kernel_size=5, sigma=3.5):
    # inputs: batch, channel, depth, height, width

    filter_1d = cv2.getGaussianKernel(kernel_size, sigma).squeeze()
    filter_2d = np.multiply(filter_1d[:, np.newaxis], filter_1d[np.newaxis, :])
    filter_3d = np.multiply(filter_2d[:, :, np.newaxis], filter_1d[np.newaxis, np.newaxis, :])
    filter = np.float32(filter_3d)
    filter = filter[np.newaxis, np.newaxis, ...]
    kernel = torch.FloatTensor(filter).cuda(input.get_device())
    if kernel.shape[1] != input.shape[1]:
        kernel = kernel.repeat(input.shape[1], 1, 1, 1)
    low = F.conv3d(input, kernel, padding=(kernel_size - 1) // 2, groups=input.shape[1])
    high = input - low
...
class MixUp_AUG_PI_3D:
    ...
    def aug(self, rgb_gt, rgb_noisy, prior):
        ...
        lam = self.dist.rsample((bs, 1)).view(-1, 1, 1, 1, 1).cuda()
